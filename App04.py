...
def crop_matches_criteria(attributes, sun_requirement, spacing_value, gdd_avail_value):
    ...
    crop_spacing = attributes.get('row_spacing')
...
def display_bar_chart(attributes):
    ...
    x_values = [0, 1]  # Two bars: 0 for Average, 1 for Plant
    y_values = [average_spread, plant_spread]
    plt.bar(x_values, y_values, color=['grey', 'green'], tick_label=["Average", "Plant"])
    plt.ylabel("Spread (cm)")
    plt.title(f"Spread Comparison for {attributes.get('name', 'Unknown Plant')}")
    st.pyplot(plt)

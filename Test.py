...
def like_crop(name, description):
    ...
    if 'liked_crops' not in st.session_state:
        st.session_state ['liked_crops'] = []

    if name not in [crop['name'] for crop in st.session_state.liked_crops]:
        st.session_state.liked_crops.append({'name': name, 'description': description})
        st.success(f" You like {name}!")
    else:
        st.warning(f"{name} is already in your liked list!")

def display_liked_crops():
    st.title('Your Liked Crops 💖')
    if 'liked_crops' in st.session_state and st.session_state.liked_crops:
        for crop in st.session_state.liked_crops:
            st.subheader(crop['name'])
            st.write(crop['description'])
            st.write('---')
    else:
        st.write("You haven't liked any crops yet. Start exploring and liking!")

...
# Funktion zum Verarbeiten der Lichtbedingung
def process_light_condition(light_condition, le_sun, space, model):
    # Normalisierung der Lichtbedingung
...
def process_light_condition(light_condition, le_sun, space, model):
    # Mapping zwischen generierten und im LabelEncoder enthaltenen Lichtbedingungen
    light_mapping = {
        "full sun": "Full Sun",
    ...
    if uploaded_image:
        try:
            ...
            if model is not None and le_sun is not None:
                # Pflanze basierend auf Lichtbedingung und Platz empfehlen

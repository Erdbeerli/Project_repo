...
# MACHINE LEARNING
# The following code on Machine Learning was fully generated by ChatGPT
# It was optimized and adjusted when needd based on error statements and other issues
...
def process_light_condition(light_condition, le_sun, space, model):
    ...
    if light_condition in le_sun.classes_:
        ...
        st.warning(f"Light condition '{light_condition}' not recognized.")
        return None

# Function that analyses the image
...
def process_light_condition(light_condition, le_sun, space, model):
    ...
    if mapped_light_condition in le_sun.classes_:
        ...
        st.warning(f"Light condition '{mapped_light_condition}' not recognized.")
        return None
    ...
    if uploaded_image:
        try:
            ...
            if model is not None and le_sun is not None:
                # Recommended plant based on the light conditions
                predicted_plant = process_light_condition(light_condition, le_sun, space, model)
                if predicted_plant:
                    st.subheader(f"🌱 Recommended Plant: {predicted_plant}")
                else:
                    st.warning("Light condition not recognized.")
            else:
                st.error("Model or LabelEncoder is not initialized.")
        except Exception as e:
